...
void song2mid(int songNum, long ptrList[4], long nextPtr, int curSpeed);
...
{
	printf("David Whittaker (GB/GBC) to MIDI converter\n");
	if (args != 3)
	{
		printf("Usage: DW2MID <rom> <bank>\n");
		return -1;
	}
	else
	{
		...
		if (tableOffset != 0)
		{
			...
			while ((i < firstPtrs[0]) && (i < firstPtrs[1]) && (i < firstPtrs[2]) && ReadLE16(&romData[i + 2 - bankAmt]) != 0 && (ReadLE16(&romData[i + 1 - bankAmt]) < (bankSize * 2)))
			{
				...
				song2mid(songNum, songPtrs, nextPtr, curSpeed);
				i += 9;
				songNum++;
			}
		}
